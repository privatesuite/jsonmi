...
(async () => {
	...
	async function (req, res) {
		...
		if (requestedUrl.pathname === "/send_email") {
			...
			const emails = (await mailee.database.getEmails()).filter(_ => (_.metadata.to.value.map(_ => _.address).indexOf(`${jwt.username}@${config.smtp.host}`) !== -1) || (_.metadata.cc ? _.metadata.cc.value.map(_ => _.address).indexOf(`${jwt.username}@${config.smtp.host}`) !== -1 : false)).slice(0, limit);
